...
public class Schemas {
    ...
    /** Read by path: the tests run from the module directory, and a test module cannot see the main module's resources. */
    public static final JsonSchema PROV_JSON = load(SpecVersion.VersionFlag.V4, "src/test/resources/prov-json-schema-v4.json");
    public static final JsonSchema PROV_JSONLD = load(SpecVersion.VersionFlag.V7, "src/main/resources/2024-08-25/jsonldschema.json");

    static JsonSchema load(SpecVersion.VersionFlag version, String file) {
        try (InputStream in = new java.io.FileInputStream(file)) {
            return JsonSchemaFactory.getInstance(version).getSchema(in, NO_FORMAT_ASSERTIONS);
        } catch (IOException e) {
            throw new IllegalStateException("schema not readable: " + file, e);
        }
    }
