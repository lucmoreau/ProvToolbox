...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml">
  ...
  <xsl:template match="/">
    <head>
      <!-- <base href="http://localhost:8080/cxf-1.0.3-SNAPSHOT/"/> -->
      <xsl:element name="base">
        <xsl:attribute name="href">
          <xsl:value-of select="$base.path" />
        </xsl:attribute>
      </xsl:element>
      <link href="../src/main/webapp/css/XMLPrettyPrint/XMLPrettyPrint.css" type="text/css" rel="stylesheet"/>
      <link title="Monospace" href="../src/main/webapp/css/XMLPrettyPrint/XMLMonoPrint.css" type="text/css" rel="alternate stylesheet"/>
  ...
  <xsl:template match="*[node()]">
    <div>
      ...
      <span class="text">
        <xsl:value-of select="."/>
        <xsl:if test="local-name(.) = 'queryId'">
          <xsl:text> [</xsl:text>
          <xsl:element name="a">
            <xsl:attribute name="href">
  ...
  <xsl:template match="*[* or processing-instruction() or comment() or string-length(.) &gt; 50]">
    ...
    <div class="expander-open">
      ...
      <div class="expander-content">
        ...
        <xsl:if test="local-name(.) = 'effectId'">
          <xsl:text> [</xsl:text>
          <xsl:element name="a">
            <xsl:attribute name="href">
              <xsl:text>browse/</xsl:text>
        ...
        <xsl:if test="local-name(.) = 'causeId'">
          <xsl:text> [</xsl:text>
          <xsl:element name="a">
            <xsl:attribute name="href">
              <xsl:text>browse/</xsl:text>
    ...
    <xsl:choose>
      <xsl:when test="name(.) = 'id'">
        ...
        <xsl:element name="a">
          <xsl:attribute name="href">
            <xsl:text>browse/</xsl:text>
            <xsl:value-of select="../*[local-name()='label']/@value" />
          </xsl:attribute>
          <span class="hlink">
            <xsl:text>browse</xsl:text>
          </span>
        ...
        <xsl:if test="local-name(..) = 'artifact'">
            <xsl:text>, </xsl:text>
            <span class="provenance">
              <xsl:text>provenance: </xsl:text>
            </span>
