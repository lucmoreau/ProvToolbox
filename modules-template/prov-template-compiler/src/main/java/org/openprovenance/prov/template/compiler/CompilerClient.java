...
public class CompilerClient {
    ...
    JavaFile generateClientLib_aux(Document doc, Set<QualifiedName> allVars, Set<QualifiedName> allAtts, String name, String templateName, String packge, String resource, JsonNode bindings_schema, IndexedDocument indexed) {
        ...
        if (bindings_schema!=null) {
           // builder.addMethod(generateClientMethod(allVars, allAtts, name, templateName, bindings_schema));
            ...
            builder.addMethod(generateClientToRecordMethod_aux(allVars, allAtts, name, templateName, compilerUtil.loggerName(templateName), packge, bindings_schema));
            builder.addMethod(generateClientApplydMethod_aux(allVars, allAtts, name, templateName, compilerUtil.loggerName(templateName), packge, bindings_schema));
 ...
 //       return generateClientMethod_aux(allVars,allAtts,name,template, compilerUtil.sqlName(template),compilerUtil.sqlName(template), bindings_schema);
  //  }
    ...
    public MethodSpec generateClientToRecordMethod_aux(Set<QualifiedName> allVars, Set<QualifiedName> allAtts, String name, String template, String loggerName, String packge, JsonNode bindings_schema) {
        ...
        MethodSpec.Builder builder = MethodSpec.methodBuilder(ConfigProcessor.ARGS2RECORD_CONVERTER)
                .addModifiers(Modifier.PUBLIC)
                .returns(processorClassName);
        if (debugComment) builder.addComment("Generated by method $N", getClass().getName()+".generateClientToRecordMethod_aux()");
        ...
        builder.addStatement("return (" + args + ") -> {  return new Object [] { " + args2 + "}; }");
    ...
    public MethodSpec generateClientApplydMethod_aux(Set<QualifiedName> allVars, Set<QualifiedName> allAtts, String name, String template, String loggerName, String packge, JsonNode bindings_schema) {
        ...
        if (debugComment) builder.addComment("Generated by method $N", getClass().getName()+".generateClientApplydMethod_aux()");

        CodeBlock.Builder jdoc = CodeBlock.builder();
        jdoc.add(loggerName + " apply method\n");
