...
public class CompilerSQL {
    ...
    public void generateSQLInsertFunctionWithSharing(String jsonschema, String templateName, String root_dir, TemplateBindingsSchema templateBindingsSchema, List<String> shared) {
        ...
        if (debugComment) res.append("-- Generated by method ").append(getClass().getName()).append(".generateSQLInsertFunctionWithSharing()\n");
        ...
        for (String key: descriptorUtils.fieldNames(templateBindingsSchema)) {
            ...
            if (descriptorUtils.isInput(key, templateBindingsSchema) || isShared) {
                first = sepIfNotFirst(res, first);
        ...
        first=true;
        for (String key: descriptorUtils.fieldNames(templateBindingsSchema)) {
            if (descriptorUtils.isOutput(key, templateBindingsSchema) && !shared.contains(key)) {
                first = sepIfNotFirst(res, first);
                final String sqlType = convertToSQLType(compilerUtil.getJavaTypeForDeclaredType(var, key).getName());
                res.append(" ").append(sqlify(key)).append(" ").append(sqlType);
            }
        ...
        first=true;
        for (String key: descriptorUtils.fieldNames(templateBindingsSchema)) {
            if (descriptorUtils.isOutputName(key, templateBindingsSchema) && !shared.contains(key)) {
                if (first) {
                    if (debugComment) res.append(" -- create new identifiers for outputs\n");
                    res.append("WITH\n");
        ...
        res.append("\nINSERT INTO ").append(HACK_orig_templateName).append("(");
        first=true;
        for (String key: descriptorUtils.fieldNames(templateBindingsSchema)) {
            first = sepIfNotFirst(res, first);
            res.append(" ").append(sqlify(key));
        ...
        for (String key: descriptorUtils.fieldNames(templateBindingsSchema)) {
            ...
            if (descriptorUtils.isInput(key, templateBindingsSchema)) {
                ...
                boolean isShared = shared.contains(key);
                if (isShared) {
                    appendPossiblySharedOutput(res, key, isShared);
                } else {
                    String the_table = descriptorUtils.getOutputSqlTable(key, templateBindingsSchema).orElse(key);
                    String new_table = "new_" + key + "_with_id";
                    String new_id = the_table + "_id";
        ...
        res.append("RETURNING ");
        if (withRelationId) {
            res.append(HACK_orig_templateName).append(".").append(tableKey).append(", ");
        }
        first=true;
        for (String key: descriptorUtils.fieldNames(templateBindingsSchema)) {
            if (descriptorUtils.isOutput(key, templateBindingsSchema) && !shared.contains(key)) {
                first = sepIfNotFirst(res, first);
                res.append(HACK_orig_templateName).append(".").append(key);
            }
        }
        res.append(";"); // end returning
    ...
    private StringBuilder appendPossiblySharedOutput(StringBuilder res, String key, boolean isShared) {
        return res.append(isShared ? ConfigProcessor.SHARED_PREFIX : ConfigProcessor.INPUT_PREFIX).append(sqlify(key));
    }
