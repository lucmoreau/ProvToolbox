...
public class TemplateQuery {
    ...
    String recursiveQuery="CREATE OR REPLACE FUNCTION backwardtraversal_star(${symbol_escape}n" +
            "    __param_id integer, ${symbol_escape}n" +
            "    __param_template text, ${symbol_escape}n" +
            "    __param_property text${symbol_escape}n" +
            ") RETURNS TABLE(${symbol_escape}n" +
            "    in_id integer, ${symbol_escape}n" +
            "    in_template text, ${symbol_escape}n" +
            "    in_property text, ${symbol_escape}n" +
            "    out_id integer, ${symbol_escape}n" +
            "    out_template text, ${symbol_escape}n" +
            "    out_property text${symbol_escape}n" +
            ")${symbol_escape}n" +
            "AS ${symbol_dollar}${symbol_dollar}${symbol_escape}n" +
            "WITH RECURSIVE recurse_traverse AS (${symbol_escape}n" +
            "    -- Initial query from backwardTraversal function with an additional column for recursion depth${symbol_escape}n" +
            "    SELECT in_id, in_template, in_property, out_id, out_template, out_property, 1 AS depth${symbol_escape}n" +
            "    FROM backwardTraversal(__param_id, __param_template, __param_property)${symbol_escape}n" +
            "${symbol_escape}n" +
            "    UNION ${symbol_escape}n" +
            "${symbol_escape}n" +
            "${symbol_escape}t-- Recursive step using the output of backwardTraversal called on new parameters obtained from predecessor_table${symbol_escape}n" +
            "    SELECT bt.in_id, bt.in_template, bt.in_property, bt.out_id, bt.out_template, bt.out_property, rt.depth + 1 AS depth${symbol_escape}n" +
            "    FROM recurse_traverse rt${symbol_escape}n" +
            "    JOIN predecessor_table pt ON rt.out_template = pt.template AND rt.out_property = pt.output${symbol_escape}n" +
            "    CROSS JOIN LATERAL backwardTraversal(rt.out_id, rt.out_template, pt.input) AS bt${symbol_escape}n" +
            "    WHERE pt.input IS NOT NULL AND rt.depth < 100 -- Ensuring pt.input is not null and limiting recursion depth${symbol_escape}n" +
            "${symbol_escape}n" +
            ")${symbol_escape}n" +
            "SELECT in_id, in_template, in_property, out_id, out_template, out_property${symbol_escape}n" +
            "FROM recurse_traverse${symbol_escape}n" +
            "${symbol_dollar}${symbol_dollar} LANGUAGE SQL;";

    /* now provides an output for the template, rather than input. */
    String recursiveQuery2="CREATE OR REPLACE FUNCTION backwardtraversal_star(${symbol_escape}n" +
            "    __param_id integer, ${symbol_escape}n" +
            "    __param_template text, ${symbol_escape}n" +
            "    __param_property text${symbol_escape}n" +
            ") RETURNS TABLE(${symbol_escape}n" +
            ...
            "WITH RECURSIVE recurse_traverse AS (${symbol_escape}n" +
            "    -- Initial query from backwardTraversal function with an additional column for recursion depth${symbol_escape}n" +
            "${symbol_escape}t${symbol_escape}n" +
            "    SELECT ${symbol_escape}n" +
            "        bt.in_id, ${symbol_escape}n" +
            "        bt.in_template, ${symbol_escape}n" +
            "        bt.in_property, ${symbol_escape}n" +
            "        bt.out_id, ${symbol_escape}n" +
            "        bt.out_template, ${symbol_escape}n" +
            "        bt.out_property, ${symbol_escape}n" +
            "        1 AS depth${symbol_escape}n" +
            "    FROM ${symbol_escape}n" +
            "        predecessor_table as pt ${symbol_escape}n" +
            "        CROSS JOIN LATERAL backwardTraversal(__param_id, __param_template, pt.input) AS bt${symbol_escape}n" +
            "    WHERE ${symbol_escape}n" +
            "        pt.template = __param_template ${symbol_escape}n" +
            "        AND pt.output = __param_property${symbol_escape}n" +
            "${symbol_escape}t${symbol_escape}n" +
            "    UNION ${symbol_escape}n" +
            "${symbol_escape}n" +
            "${symbol_escape}t-- Recursive step using the output of backwardTraversal called on new parameters obtained from predecessor_table${symbol_escape}n" +
    ...
    public void generateViz(Integer id, String template, String property, String style, Map<String, Map<String, String>> baseTypes, String principal, OutputStream out) {
        ...
        //logger.info("templateConnections: " + templateConnections.stream().map(TemplateConnection::toString).collect(Collectors.joining("${symbol_escape}n")));
    ...
    public List<TemplateConnection> recursiveTraversal(Integer id, String template, String property, String principal) {
        ...
        querier.do_query(the_records,
                ...
                (sb, data) -> {
                    ...
                    sb.append("','");
                    sb.append(property);
                    sb.append("') as template_connection${symbol_escape}n");
    ...
    public List<Object[]> querySimple(String template, Integer id, boolean withTitles, String principal) {
        ...
        querier.do_query(the_records,
             ...
             */
                    ...
                    sb.append("SELECT template.*${symbol_escape}n FROM ");
                    sb.append(template);
                    sb.append(" as template ");
                    joinAccessControl(template, principal, sb);
                    sb.append("${symbol_escape}n WHERE template.id=");
                    sb.append(id);
                    andAccessControl(principal, sb);
    ...
    public void joinAccessControl(String template, String principal, StringBuilder sb, String label, String id) {
        sb.append("${symbol_escape}n LEFT JOIN record_index ON record_index.key=" + label + "." + id);
        if (template.startsWith(label)) {
            sb.append("${symbol_escape}n AND record_index.table_name=");
            sb.append(template);
        } else {
            sb.append("${symbol_escape}n AND record_index.table_name='");
            sb.append(template);
        ...
        sb.append("${symbol_escape}n AND record_index.principal IS NOT NULL");
        sb.append("${symbol_escape}n LEFT JOIN access_control${symbol_escape}n ON access_control.record=record_index.id");
        sb.append("${symbol_escape}n AND access_control.authorized='");
        sb.append(principal);
        sb.append("'");
    }
    ...
    private List<RecordEntry2> queryTemplatesRecords(String base_relation, String from_date, String to_date, Integer limit, String principal, boolean includeComposite) {
        ...
        querier.do_query(linked_records,
                ...
                (sb, data) -> {
                    ...
                    sb.append("SELECT search_record.*,record_index.principal,json_agg(ac2.authorized) as authorized,").append(aggregator.toString()).append(" as parent, record_index.hash as hash${symbol_escape}n FROM ");
                    ...
                    sb.append("${symbol_escape}n LEFT JOIN access_control as ac2  ON ac2.record=record_index.id");
                    final int[] count = {0};
                    compositeLinker.forEach((k,v) -> {
                        if (includeComposite) {
                            count[0]++;
                            sb.append("${symbol_escape}n LEFT JOIN ");
                            sb.append(v.table);
                            sb.append(" as link").append(count[0]);
                            sb.append("${symbol_escape}n ON search_record.key=link").append(count[0]).append(".simple");
                        }
                    });

                    //sb.append("${symbol_escape}n LEFT JOIN plead_transforming_composite_linker as link${symbol_escape}n");
                   // sb.append("${symbol_escape}n ON record_index.key=simple");
    ...
    public static class RecordEntry2 {
        ...
        public String toString() {
            return "RecordEntry2{" +
                    "key=" + key +
                    ", property='" + property + '${symbol_escape}'' +
                    ", base_relation='" + base_relation + '${symbol_escape}'' +
                    ", created_at='" + created_at + '${symbol_escape}'' +
                    ", table_name='" + table_name + '${symbol_escape}'' +
                    ", id=" + id +
                    ", principal='" + principal + '${symbol_escape}'' +
    ...
    public List<Object[]> queryComposite(String template, Integer id, boolean withTitles, String principal) {
        ...
        querier.do_query(linked_records,
                ...
                (sb, data) -> {
                    ...
                    sb.append(" AS template ");
                    joinAccessControl(template, principal, sb);
                    sb.append("${symbol_escape}n WHERE key=");
                    sb.append(id);
                    andAccessControl(principal, sb);
                    sb.append(") as combo ");
                    sb.append("${symbol_escape}n LEFT JOIN ").append(linker.table).append(" AS linker ");
