...
public class RdfConstructor implements TreeConstructor {
	...
	//TODO: bundle name does not seem to be interpreted according to the prefix declared in bundle.
	//TODO: handle prefix declarations
	QName qname = getQName(bundleId);
	if (qname!=null) {
	    ((SesameManager)manager).getConnection().setAddContexts(new URIImpl(qname.getNamespaceURI()
	                                                                        +qname.getLocalPart()));
	}
	...
	for (Object entry : aAttrs) {
		...
		if (typeAsString.equals("xsd:QName")) { // TODO: this should
							// become prov:qualified
							// name
		    System.out.println("----> " + value);
		    Class<?> cl = reservedClass(value);
		    if (cl != null)
			manager.designate(a, cl);
	...
	if (qn3 != null) {
	    Plan plan = (Plan) manager.designate(qn3, Plan.class); 
