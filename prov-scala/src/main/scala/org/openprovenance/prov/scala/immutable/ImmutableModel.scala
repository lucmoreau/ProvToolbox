...
trait Identifiable extends org.openprovenance.prov.model.Identifiable {
    ...
    def setId(x: org.openprovenance.prov.model.QualifiedName) {
      throw new UnsupportedOperationException
    }
    ...
    def idOrMarker(id: QualifiedName,sb: StringBuilder)={
      if (id!=null) {sb++=id.toString()} else {sb+='-'}
    }
    
    def timeOrMarker(time: XMLGregorianCalendar,sb: StringBuilder) {
        if (time == null) { sb+='-' } else { sb++= time.toString()};
    }
    def timeOrMarker(time: Option[XMLGregorianCalendar],sb: StringBuilder) {
...
trait ImmutableMentionOf extends Relation with org.openprovenance.prov.model.MentionOf with org.openprovenance.prov.model.extension.QualifiedSpecializationOf with Identifiable  with HasLabel with HasType with HasOther with HasAttributes with Hashable {
  ...
  @BeanProperty
    ...
    def setGeneralEntity(x$1: org.openprovenance.prov.model.QualifiedName) = throw new UnsupportedOperationException
    def setSpecificEntity(x$1: org.openprovenance.prov.model.QualifiedName)= throw new UnsupportedOperationException
    def setBundle(x$1: org.openprovenance.prov.model.QualifiedName)= throw new UnsupportedOperationException

       
    def canEqual(a: Any) = a.isInstanceOf[ImmutableMentionOf]
...
abstract class Node extends Statement with HasType with HasOther {
      ...
      @inline final def ll(m: Map[String,String], s: Set[LangString]) = {
          if (s==null) null else s.map(l => l.rename(m))
      }
      
  
  def rename(m: Map[QualifiedName,QualifiedName], lab: Map[String,String]) = {
    this match {
...
class WasDerivedFrom(val id: QualifiedName,
      ...
      override def rename(m: Map[QualifiedName,QualifiedName])={
    ...
    e match {
      ...
      case _ =>
            new WasGeneratedBy(QualifiedName(e.getId),
                               ...
                               QualifiedName(e.getActivity),
                               if (e.getTime==null) None else Some(e.getTime),
                               LangString(e.getLabel),
                               Type(e.getType),
                               Location(e.getLocation),
                               Role(e.getRole),
...
object Attribute {    
  ...
  def toNotation (sb: StringBuilder, ls: Set[LangString],ts: Set[Type],vs: Set[Value],locs: Set[Location],rs: Set[Role],os: Map[QualifiedName,Set[Other]]) = {
...
object Type {
      val myName=QualifiedName(ProvFactory.pf.getName.PROV_TYPE)
...
object Role {
      val myName=QualifiedName(ProvFactory.pf.getName.PROV_ROLE)
...
object Location {
      val myName=QualifiedName(ProvFactory.pf.getName.PROV_LOCATION)
      private[immutable] def apply (`type`: QualifiedName,
                                    value: Object) = {
...
object Value {
      val myName=QualifiedName(ProvFactory.pf.getName.PROV_VALUE)
...
class LangString (@BeanProperty val value: String,
  ...
  override def setValue(x: String) {
    ...
    def toLabel () = {
     ...
     def rename(m: Map[String,String]) = {
       m.get(value) match {
         case None => this
...
object Label {
      val myName=QualifiedName(ProvFactory.pf.getName.PROV_LABEL)
...
object Other {
      ...
      def apply(others: java.util.Collection[org.openprovenance.prov.model.Other]): Map[QualifiedName,Set[Other]] = {
          ...
          val group2=group.mapValues(_.map(_._2))
          /*
          others2.map(s => {
                       val o=Other(s);
                       o.elementName -> Set(o)
...
class Other protected (val elementName : QualifiedName,
    ...
    def setElementName (x$1: org.openprovenance.prov.model.QualifiedName) = { throw new UnsupportedOperationException }
...
class QualifiedName (val prefix: String,
    ...
    override val hashCode = this.namespaceURI.hashCode ^ this.localPart.hashCode()

    def getUri () = uri

    def getLocalPart()= localPart
    
    def getNamespaceURI() = namespaceURI
    
    def getPrefix() = prefix

    def setLocalPart(x$1: String) = { throw new UnsupportedOperationException }
    
    def setNamespaceURI(x$1: String) =  { throw new UnsupportedOperationException }
    
    def setPrefix(x$1: String) = { throw new UnsupportedOperationException }
    
    override def setUri(discard: String) = { throw new UnsupportedOperationException  }
    ...
    def canEqual(a: Any) = a.isInstanceOf[QualifiedName]
...
trait ImmutableConstructor extends ImmutableConstructorInterface {
  ...
  override def newActivity (id: QualifiedName,
    ...
    newActivity(id,
      if (startTime==null) None else Some(startTime),
      if (endTime==null) None else Some(endTime),
      attributes)
  }
  ...
  override def newWasInformedBy (id: QualifiedName,
    ...
    split(attributes) match {
      case (ls,ts,vs,locs,rs,os) =>
        ...
        if (size>0) throw new ValueExistException
        if (rs.size>0) throw new RoleExistException
        if (locs.size>0) throw new LocationExistException
        val langStrings=LangString(ls.map(l=>l.getLangString()))
...
class ProvFactory extends ProvFactory1  {
  ...
  override def newDocument(document: org.openprovenance.prov.model.Document) = {
